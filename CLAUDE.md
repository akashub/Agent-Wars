...
- **Agent runtime**: Claude Agent SDK (Python) for the battle runner — composes resolved agents, runs the loop, enforces budgets.
- **LLM**: Anthropic Claude. Models read from env; default to latest (Opus 4.8 `claude-opus-4-8`, Sonnet 4.6 `claude-sonnet-4-6`, Haiku 4.5 `claude-haiku-4-5-20251001`). The judge model is always independent of competitor models.
